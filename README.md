...
- Telegram Bot API не поддерживает телефонные звонки боту. Для настоящих звонков нужен внешний voice/SIP-провайдер. Основной путь для живого диалога с русскими номерами: Asterisk на сервере звонит через SIPNET, а `secretary-ai` Worker делает ASR, выбор следующей реплики и финальный разбор. Voximplant, MTS Exolve и Twilio оставлены как старые опциональные адаптеры.
...
- `VOICE_BUSINESS_CALL_PROVIDER=sipnet` запускает живой диалог через локальный Asterisk: приложение пишет JSON-задачу в `/app/asterisk/tasks`, Asterisk читает её как `/var/lib/telegram-secretary/asterisk/tasks`, звонит через SIPNET, говорит через TTS, отправляет ответы в `/asr` Worker и после разговора отправляет финальный разбор в Telegram.
- Для SIPNET/Asterisk нужны `VOICE_BUSINESS_CALLS_ENABLED=true`, `VOICE_BUSINESS_CALL_PROVIDER=sipnet`, `ASTERISK_AMI_HOST`, `ASTERISK_AMI_USERNAME`, `ASTERISK_AMI_PASSWORD`, `ASTERISK_TASK_DIR`, `ASTERISK_HOST_TASK_DIR`, `LLM_WORKER_URL` и `LLM_WORKER_BEARER_TOKEN`.
- `VOICE_BUSINESS_CALL_PROVIDER=voximplant_dialog` запускает старый живой диалог через Voximplant: Voximplant звонит, слушает через ASR, спрашивает следующие вопросы через `secretary-ai` Worker и после разговора отправляет финальный разбор в Telegram, если в Worker заданы `TELEGRAM_BOT_TOKEN` и `SECRETARY_OWNER_TELEGRAM_ID`.
...
- Для запуска через внешний SIP-транк нужны `VOXIMPLANT_OUTBOUND_TRANSPORT=sip`, `VOXIMPLANT_SIP_URI_TEMPLATE` и, если оператор требует авторизацию, `VOXIMPLANT_SIP_AUTH_USER` плюс `VOXIMPLANT_SIP_PASSWORD_SECRET_NAME`. SIP URI можно собирать из `{destination}`, `{destination_digits}`, `{destination_local}`, `{destination_trunk8}`. Для SIPNET рабочий путь: создать Voximplant SIP Registration на `sipnet.ru`, указать ее id в `VOXIMPLANT_SIP_REG_ID`, поставить `VOXIMPLANT_SIP_URI_TEMPLATE={destination_digits}` и `VOXIMPLANT_SIP_CALLER_ID=логин@sipnet.ru`. Прямой `sip:{destination_digits}@sipnet.ru` может возвращать SIP `404 unknown user account`, хотя SIP REGISTER и API SIPNET проходят успешно.
